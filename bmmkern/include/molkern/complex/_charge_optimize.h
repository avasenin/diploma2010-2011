  ...
  private:
	...
	real_t run(LPComplex *complex, const Param &param)
    {
      ...
      for (int k=0; k < 200; k++)
      {
        for (int i=0; i < 10; i++)
        {
          modeling(m_variables, 0.01, params);
        }
        PRINT("BING");
        for (int i=0; i < number_of_atoms; i++)
        {
        	avarage[i] = (avarage[i] * k + m_variables[i]) / (k + 1);
        	own << setiosflags(ios::fixed) << setprecision(5) <<  avarage[i] << " ";
        }
        own << endl;
      }
      own.close();
...
//normalization
      ...
      for (unsigned moleculeIndex = 0; moleculeIndex < numberOfMolecules; moleculeIndex++)
      {
        ...
        dp_dt[numberOfAtomsInMolecule + atomOffset -1] = 0.;
