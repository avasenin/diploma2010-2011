    ...
    void buildMatrixAKA()
    {
      ...
      for (unsigned moleculeIndex = 0; moleculeIndex < numberOfMolecules; moleculeIndex++)
      {
        ...
        for (unsigned bondIndex = 0; bondIndex < currentArchetype->count(BOND); bondIndex++)
        {
          ...
          unsigned i = currentArchetype->get(BOND)[bondIndex].ndx[0]+atomOffset;
          unsigned j = currentArchetype->get(BOND)[bondIndex].ndx[1]+atomOffset;
          m_AKA(i,j) = -m_K(k,k);
          m_AKA(j,i) = -m_K(k,k);
          m_AKA(i,i) += m_K(k,k);
          m_AKA(j,j) += m_K(k,k);
   ...
   //   printMatrix(m_AKA, "Matrix AKA");
    ...
    void buildMatrixK()
    {
      ...
      for (unsigned bondIndex = 0; bondIndex < m_numberOfBonds; bondIndex++)
      {
        ...
        bool isIncludedToSite = atoms[firstAtomIndexInCurrentBond].includedToActiveSite && atoms[secondAtomIndexInCurrentBond].includedToActiveSite;
        if (true || isIncludedToSite)
        {
