...
namespace prgkern
{
	...
	/**
	 * Конструкция assert(_EQ(r, s)) может быть применена не только для проверки значений, но и также
	 * для проверки корректности выполнения той или иной функции (функционального объекта), например
	 *   assert( _EQ(1, s) )        - проверка значения переменной s
	 *   assert( _EQ(1, f()) )      - проверка значения, которое возвращает функция f()
	 *   assert( _EQ(1, obj(2)()) ) - проверка значения, которое возвращает объект obj(2)
	 */
