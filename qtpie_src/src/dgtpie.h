...
inline void dsolver(const Molecule &Mol, const double *b, int N, double *x, double CoulIntMaxR)
{
	...
	int k,i;
	double dimension(N) :: r, p, q, z, tmp;
	...
	norm = dnrm2(N, r, 1);
	if (Verbose)
		printf("Iteration 0 : norm = %lf %lf", norm, norm/critical_norm);
	...
	for (int k=0; k<max_k; k++)
	{
		...
		if (Verbose) printf("Iteration %d : %lf %lf", k, norm, norm/critical_norm);
...
label1:
	if (Verbose)
		printf("dcg: solution found with residual %lf", norm);
}
...
*/
void MultiplyByA(const Molecule &Mol, const double *X, int N, double *V, double CoulIntMaxR)
{
...
//c$omp   end parallel do
	...
	dsolver(Mol, Voltage, N+1, CoulIntMaxR);
	...
	for (int i1=0; i1<N; i1++)
	{
		...
		for (int i2=0; i2<i1; i2++)
			Mol.Energy += ThisCharge * Mol.Atoms[i2].Charge * Mol.Coulomb(i1, i2);
		Mol.Energy += 0.5 * ThisCharge * ThisCharge * Mol.Coulomb(i1, i1);
	}
